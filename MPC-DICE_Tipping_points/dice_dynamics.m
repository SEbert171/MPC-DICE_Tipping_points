...
%states of the tipping points (before the obective)
Yedoma_triggered=x(15);
Amazon_triggered = x(16);
Greenland_triggered = x(17);
...
Yedoma_threshold=Params.Yedoma_threshold;%see assign parameters function
Yedoma_impact=Params.Yedoma_impact;

Amazon_threshold=Params.Amazon_threshold;
Amazon_impact = Params.Amazon_impact;

Greenland_threshold=Params.Greenland_threshold;
Greenland_impact=Params.Greenland_impact;
...
% ---------------Triggering of Tipping points -------------------


%Yedoma

if(strcmp(class(Yedoma_triggered),char('double')))
                                        %differentiate between the case where
                                        %we work with Casadi.SX variables
                                        %and the case the variables are
                                        %doubles
      if(Yedoma_triggered==1||T_AT>=Yedoma_threshold)                                      
        Yedoma_triggered=1;
      else
          Yedoma_triggered=0;
      end
else
    Yedoma_triggered = if_else((T_AT>=Yedoma_threshold|Yedoma_triggered==SX(1)),SX(1), SX(0),false);
end


%Amazon

if(strcmp(class(Amazon_triggered),char('double')))
                                        %differentiate between the case where
                                        %we work with Casadi.SX variables
                                        %and the case the variables are
                                        %doubles
      Amazon_emission=0;
      if(Amazon_triggered==0&&T_AT>=Amazon_threshold)   %Determination if the tipping point will be triggered in this time step
            Amazon_emission=Amazon_impact;
      end
      if(Amazon_triggered==1||T_AT>=Amazon_threshold)                                      
        Amazon_triggered=1;
      else
          Amazon_triggered=0;
      end
else
    Amazon_emission = if_else((Amazon_triggered==SX(0)&T_AT>=Amazon_threshold),SX(Amazon_impact), SX.zeros(1,1),false);
    Amazon_triggered = if_else((T_AT>=Amazon_threshold|Amazon_triggered==SX(1)),SX(1), SX(0),false);
end
...
if(strcmp(class(Greenland_triggered),char('double')))
                                        %differentiate between the case where
                                        %we work with Casadi.SX variables
                                        %and the case the variables are
                                        %doubles
      if(Greenland_triggered==1||T_AT>=Greenland_threshold)                                      
        Greenland_triggered=1;
      else
          Greenland_triggered=0;
      end
        Greenland_economic_impact=1-(Greenland_impact*Greenland_triggered);
else
    Greenland_triggered = if_else(T_AT>=Greenland_threshold|Greenland_triggered==SX(1),SX(1), SX(0),false);
    Greenland_economic_impact=(SX(1)-(SX(Greenland_impact)*Greenland_triggered));
end
...
% ---------------- Carbon Cycle -------------------
M_NEXT = Phi_M * M + [xi2*E; 0; 0]+[Amazon_emission;0;0]+[Yedoma_triggered*Yedoma_impact;0;0];%no
...
Net_Economic_Output_NEXT = Damages_NEXT *(1 - theta1_NEXT*(mu_NEXT^theta2))*...
    Gross_Economic_Output_NEXT*Greenland_economic_impact;      
...
f = [i_NEXT; T_NEXT; M_NEXT; K_NEXT; ...
     ...
     E_NEXT; C_NEXT; Yedoma_triggered; Amazon_triggered; Greenland_triggered; mu_NEXT; s_NEXT;  J_NEXT];%