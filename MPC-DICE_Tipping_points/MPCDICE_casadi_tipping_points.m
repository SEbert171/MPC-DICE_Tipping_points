function  F =     MPCDICE_casadi_tipping_points(Yedoma_threshold,Yedoma_impact, Amazon_threshold,Amazon_impact, Greenland_threshold, Greenland_impact)
...
%assigning the parameters for the tipping points
    Params.Yedoma_impact=Yedoma_impact;     
    Params.Yedoma_threshold=Yedoma_threshold;

    Params.Amazon_impact=Amazon_impact;
    Params.Amazon_threshold=Amazon_threshold;

    Params.Greenland_impact=Greenland_impact;
    Params.Greenland_threshold=Greenland_threshold;
...
Data.tf = 60;       % final simulation time; set tf = 1 to solve single OCP
Data.nx = 1+6+9+3+1;  % total # of states: time; 6 "endogenous" states; 
                    %   9 auxilliary states including 5 "exogenous" states,
                    %   consumption, emissions, and shifted inputs; 
                    % states of tipping points and the objective
Data.nu = 2;        % # of inputs
...
switch Data.tf
    case 1 % Solve single OCP
        ...
        lambda = lambda(1:end-18);%STILL ADJUST??? -18???? before it was -16
    ...
    otherwise %% Run MPC loop           
        ...
        for k = Data.t0 : Data.step : Data.tf
            ...
            lambda  = full(res.lam_g);
            if k == Data.t0
                lambda = lambda(1:end-18);%?? normally -15
                lambda = reshape(lambda, Data.nx, Data.N+1);
            else
                lambda = reshape(lambda, Data.nx, Data.N+1);
            end
...
YEDOMA_TRIGGERED = Data.xMPC(15,:);
AMAZON_TRIGGERED = Data.xMPC(16,:);
GREENLAND_TRIGGERED = Data.xMPC(17,:);
...
SaveData.Yedoma_threshold=Params.Yedoma_threshold;
SaveData.Yedoma_impact=Params.Yedoma_impact;
SaveData.Yedoma_triggered=YEDOMA_TRIGGERED;
SaveData.Amazon_treshold=Params.Amazon_threshold;
SaveData.Amazon_impact=Params.Amazon_impact;
SaveData.Amazon_triggered=AMAZON_TRIGGERED;
SaveData.Greenland_threshold=Params.Greenland_threshold;
SaveData.Greenland_impact=Params.Greenland_impact;
SaveData.Greenland_triggered=GREENLAND_TRIGGERED;
