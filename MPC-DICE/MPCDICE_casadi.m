...
Data.tf = 100;       % final simulation time; set tf = 1 to solve single OCP
Data.nx = 1+6+9+1+1;  % total # of states: time; 6 "endogenous" states; 
...
switch Data.tf
    ...
    otherwise %% Run MPC loop           
        ...
        for k = Data.t0 : Data.step : Data.tf
            ...
            lambda  = full(res.lam_g);
            if k == Data.t0
                lambda = lambda(1:end-16);%?? normally -15
                lambda = reshape(lambda, Data.nx, Data.N+1);
            else
                lambda = reshape(lambda, Data.nx, Data.N+1);
            end
...
if Data.tf == 1
    ...
    figure(4)
    stem(years(1:length(years)),TATM)
    title('Atmospheric Temperature Anomaly')
    xlabel('Years')

    figure(5)
    stem(years,MATM)
    title('C-concentration in the atmosphere (in GtC)')
    xlabel('Years')

    figure(6)
    stem(years,YEDOMA_TRIGGERED)
    title('State of the Yedoma tipping point')
    xlabel('Years')
